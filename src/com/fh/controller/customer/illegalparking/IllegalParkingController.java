package com.fh.controller.customer.illegalparking;
...
import com.fh.service.customer.illegalparking.IllegalParkingManager;
 ...
 */
@Controller
@RequestMapping(value="/illegalparking")
public class IllegalParkingController extends BaseController {
	
	String menuUrl = "illegalparking/list.do"; //菜单地址(权限用)
	@Resource(name="illegalparkingService")
	private IllegalParkingManager illegalparkingService;
	...
	public ModelAndView save() throws Exception{
		logBefore(logger, Jurisdiction.getUsername()+"新增IllegalParking");
		if(!Jurisdiction.buttonJurisdiction(menuUrl, "add")){return null;} //校验权限
		ModelAndView mv = this.getModelAndView();
		PageData pd = new PageData();
		pd = this.getPageData();
		pd.put("ILLEGALPARKING_ID", this.get32UUID());	//主键
		illegalparkingService.save(pd);
	...
	public void delete(PrintWriter out) throws Exception{
		logBefore(logger, Jurisdiction.getUsername()+"删除IllegalParking");
		if(!Jurisdiction.buttonJurisdiction(menuUrl, "del")){return;} //校验权限
		PageData pd = new PageData();
		pd = this.getPageData();
		illegalparkingService.delete(pd);
		out.write("success");
		out.close();
	}
	...
	public ModelAndView edit() throws Exception{
		logBefore(logger, Jurisdiction.getUsername()+"修改IllegalParking");
		if(!Jurisdiction.buttonJurisdiction(menuUrl, "edit")){return null;} //校验权限
		ModelAndView mv = this.getModelAndView();
		PageData pd = new PageData();
		pd = this.getPageData();
		illegalparkingService.edit(pd);
		mv.addObject("msg","success");
		mv.setViewName("save_result");
		return mv;
	}
	...
	public ModelAndView list(Page page) throws Exception{
		logBefore(logger, Jurisdiction.getUsername()+"列表IllegalParking");
		...
		page.setPd(pd);
		List<PageData>	varList = illegalparkingService.list(page);	//列出IllegalParking列表
		mv.setViewName("customer/illegalparking/illegalparking_list");
	...
	public ModelAndView goAdd()throws Exception{
		...
		PageData pd = new PageData();
		pd = this.getPageData();
		mv.setViewName("customer/illegalparking/illegalparking_edit");
	...
	public ModelAndView goEdit()throws Exception{
		...
		PageData pd = new PageData();
		pd = this.getPageData();
		pd = illegalparkingService.findById(pd);	//根据ID读取
		mv.setViewName("customer/illegalparking/illegalparking_edit");
	...
	public Object deleteAll() throws Exception{
		logBefore(logger, Jurisdiction.getUsername()+"批量删除IllegalParking");
		...
		if(null != DATA_IDS && !"".equals(DATA_IDS)){
			...
			illegalparkingService.deleteAll(ArrayDATA_IDS);
			pd.put("msg", "ok");
		}else{
			pd.put("msg", "no");
		}
	...
	public ModelAndView exportExcel() throws Exception{
		logBefore(logger, Jurisdiction.getUsername()+"导出IllegalParking到excel");
		...
		List<PageData> varOList = illegalparkingService.listAll(pd);
